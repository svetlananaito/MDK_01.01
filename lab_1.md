...
Вывод
14
Что тренирует: переполнение при умножении и приём 1LL *. Где пригодится: статистика (дисперсия, среднеквадратичное отклонение), физика, любые суммы произведений. Как подступиться: ответ огромный — копите в long long, и само i * i считайте сразу в long long, иначе произведение переполнит int ещё до прибавления. ⭐ Бонус: проверьте себя формулой n·(n+1)·(2n+1)/6 — результат обязан совпасть с циклом (и в формуле тоже держите long long).
